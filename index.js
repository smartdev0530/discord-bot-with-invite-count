...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isButton()) {
    ...
    if (interaction.customId === "complete_third_mission") {
      const twittycordUserData = await (
        await fetch(
          `https://twittycord.com/api/getUser?key=${config.twittycordKey}&discordId=${interaction.user.id}`
        )
      ).json();

      let twitterId = twittycordUserData?.user?.connections?.find(
        (con) => con.name === "twitter"
      )?.accountId;
    ...
    if (interaction.customId === "complete_fourth_mission") {
      ...
      const inviteCount = invitesCount[interaction.user.id] || 0;
